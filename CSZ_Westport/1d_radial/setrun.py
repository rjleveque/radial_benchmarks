...
grid_type = 2
onshore_res = 1
fname_celledges = os.path.abspath(f'celledges_{onshore_res}m.txt')
...
#------------------------------
    ...
    geo_data.coordinate_system = 2  # latitude on sphere

    topo_data = rundata.topo_data
    topo_data.topofiles.append([1, fname_celledges])

    dtopo_data = rundata.dtopo_data
    dtopo_data.dtopofiles.append([1, f'gaussian5m_dtopo_{onshore_res}m.dtt1'])
    ...
    if 1:
        rm_shore = 2518.589 * 1e3
        rm_gauges_toshore = range(-400, 2501, 200)
        ...
        for k,xp_g in enumerate(lat_gauges):
            gaugeno = k+1
