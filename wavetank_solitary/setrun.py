...
d_param = 1.  # scaling parameter d
alpha = 0.1   # angle of wavetank to grid
...
#------------------------------
    ...
    clawdata.lower[0] = -20.0
    clawdata.upper[0] = 110.0

    clawdata.lower[1] = -35.0
    clawdata.upper[1] = 40.0
    ...
    if clawdata.output_style == 1:
        ...
        clawdata.num_output_times = 100 #140
        clawdata.tfinal = 30. #44.6985
    ...
    regions.append([1,2,0,1e9,-100,200,-100,100])
    regions.append([1,2,0,1e9,-100,75,-100,100])
    regions.append([1,3,10,1e9,-5,10,-100,100])

    # == setgauges.data values ==
    # for gauges append lines of the form  [gaugeno, x, y, t1, t2]
    # rundata.gaugedata.gauges.append([])
    for i,xhat in enumerate([0.25*d_param, 9.95*d_param]):
        for j,yhat in enumerate([0, 5, 10]):
            gaugeno = 100 + 10*i + j
            xg = np.cos(alpha)*xhat - np.sin(alpha)*yhat
            yg = np.sin(alpha)*xhat + np.cos(alpha)*yhat
...
#-------------------
    ...
    #    [topotype, fname]
    topo_data.topofiles.append([3, 'wavetank.asc'])
    ...
    #   [fname]
    rundata.qinit_data.qinitfiles.append(['wave.xyz'])
